...
func Worker(mapf func(string, string) []KeyValue,
	...
	for {
		...
		switch task.Type {
		case MAP:
			fmt.Println("map task ", task.FileName)
...
func RequestTask() Task {
	...
	if ok {
		fmt.Println("get Task %s success\n", reply.Task.Type)
	} else {
...
func TaskDone(id int) {
	...
	if ok {
		fmt.Println("send Task Done success\n")
	} else {
		fmt.Println("send Task Done failed!\n")
	}
